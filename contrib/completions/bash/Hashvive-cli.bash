# bash programmable completion for hashvive-cli(1)
# Copyright (c) 2012-2022 The Hashvive developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $hashvive-cli for RPC
_hashvive_rpc() {
...
_hashvive_cli() {
    ...
    # save and use original argument to invoke hashvive-cli for -help, help and RPC
    # as hashvive-cli might not be in $PATH
    hashvive_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($hashvive_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_hashvive_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _hashvive_cli hashvive-cli
