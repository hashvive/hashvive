...
"""Script for verifying Hashvive release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from hashvive.org and hashvive.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/hashvive-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://hashvive.org"
HOST2 = "https://hashvive.org"
VERSIONPREFIX = "hashvive-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the hashvive community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"hashvive_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by hashvive.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the hashvive release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes hashvive.org lags behind hashvive.org.)')
    )
