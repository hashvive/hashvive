...
Tracing (USDT) support for Hashvive on Linux using. For more information on
USDT support in Hashvive see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `hashvived` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `hashvived` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Hashvive USDT documentation] on how to list available tracepoints in your
`hashvived` binary.

[Hashvive USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof hashvived)
```
...
hashvive P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof hashvived)
```
...
In a different terminal, starting Hashvive in SigNet mode and with
re-indexing enabled.

```
$ ./build/src/hashvived -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof hashvived)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof hashvived)
```
