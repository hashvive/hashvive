#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Hashvive developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing hashvive-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='hashvive-cli', help='hashvive-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='hashvive address to which the faucet should send')
...
parser.add_argument('hashvive_cli_args', nargs='*', help='Arguments to pass on to hashvive-cli (default: -signet)')

args = parser.parse_args()

if args.hashvive_cli_args == []:
    args.hashvive_cli_args = ['-signet']


def hashvive_cli(rpc_command_and_params):
    argv = [args.cmd] + args.hashvive_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = hashvive_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = hashvive_cli(['getnewaddress', 'faucet', 'bech32'])
