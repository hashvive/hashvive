...
const char * const HASHVIVE_CONF_FILENAME = "hashvive.conf";
const char * const HASHVIVE_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //hashvive-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", HASHVIVE_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\hashvive
    //   new: C:\Users\Username\AppData\Local\hashvive
    // macOS: ~/Library/Application Support/hashvive
    // Unix-like: ~/.hashvive
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "hashvive";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "hashvive";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/hashvive";
#else
    // Unix-like
    return pathRet / ".hashvive";
#endif
#endif
}
