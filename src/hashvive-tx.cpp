...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " hashvive-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The hashvive-tx tool is used for creating and modifying hashvive transactions.\n\n"
                "hashvive-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded hashvive transaction, or with \"-create [commands]\" to create a hex-encoded hashvive transaction.\n"
                "\n"
                "Usage: hashvive-tx [options] <hex-tx> [commands]\n"
                "or:    hashvive-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded hashvive transaction
