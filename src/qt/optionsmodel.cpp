...
/** Call node.updateRwSetting() with hashvive 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because hashvive 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/hashvive/hashvive/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because hashvive 22.x will not try to read these.
...
//! Convert enabled/size values to hashvive -prune setting.
...
//! Get pruning enabled value to show in GUI from hashvive -prune setting.
...
//! Get pruning size value to show in GUI from hashvive -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayBitcoinUnit")) {
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(BitcoinUnit::HRLV));
    }
    QVariant unit = settings.value("DisplayBitcoinUnit");
    if (unit.canConvert<BitcoinUnit>()) {
        m_display_hashvive_unit = unit.value<BitcoinUnit>();
    } else {
        m_display_hashvive_unit = BitcoinUnit::HRLV;
        settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_hashvive_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if hashvive.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_hashvive_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinUnit>() == m_display_hashvive_unit) return;
    m_display_hashvive_unit = new_unit.value<BitcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinUnit", QVariant::fromValue(m_display_hashvive_unit));
    Q_EMIT displayUnitChanged(m_display_hashvive_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/hashvive/hashvive/pull/8273
    ...
    // (https://github.com/hashvive-core/gui/issues/567).
