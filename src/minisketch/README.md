...
`libminisketch` is an optimized standalone MIT-licensed library with C API for constructing and decoding _set sketches_, which can be used for compact set reconciliation and other applications.
...
- Sketches have a predetermined capacity, and when the number of elements in the set is not higher than the capacity, `libminisketch` will always recover the entire set from the sketch. A sketch of _b_-bit elements with capacity _c_ can be stored in _bc_ bits.
- The sketches of two sets can be combined by adding them (XOR) to obtain a sketch of the [symmetric difference](https://en.wikipedia.org/wiki/Symmetric_difference) between the two sets (_i.e._, all elements that occur in one but not both input sets).
...
If the elements are large, it may be preferable to compute the sketches over _hashes_ of the set elements. In that case an additional step is added to the protocol, where Bob also sends the hash
