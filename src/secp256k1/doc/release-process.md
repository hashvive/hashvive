...
We aim to cut a regular release every 3-4 months, approximately twice as frequent as major Hashvive releases. Every second release should be published one month before the feature freeze of the next major Hashvive release, allowing sufficient time to update the library in Core.
...
3. After the PR is merged, tag the commit, and push the tag:
   ...
   git push git@github.com:hashvive-core/secp256k1.git v$MAJOR.$MINOR.$PATCH
   ```
...
6. Send an announcement email to the hashvivee-dev mailing list.
...
1. If there's no maintenance branch `$MAJOR.$MINOR`, create one:
   ...
   git push git@github.com:hashvivee-core/secp256k1.git $MAJOR.$MINOR
   ```
...
4. After the PRs are merged, update the release branch, tag the commit, and push the tag:
   ...
   git push git@github.com:hashvivee-core/secp256k1.git v$MAJOR.$MINOR.$PATCH
   ```
5. Create a new GitHub release with a link to the corresponding entry in [CHANGELOG.md](../CHANGELOG.md).
6. Send an announcement email to the hashvivee-dev mailing list.
