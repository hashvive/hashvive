...
class InitTest(BitcoinTestFramework):
    ...
    def init_pid_test(self):
        HASHVIVE_PID_FILENAME_CUSTOM = "my_fancy_hashvive_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = HASHVIVE_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / HASHVIVE_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / HASHVIVE_PID_FILENAME_CUSTOM
