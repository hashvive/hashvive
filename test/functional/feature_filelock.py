...
"""Check that it's not possible to start a second hashvived instance using the same datadir or wallet."""
import random
import string

from test_framework.test_framework import BitcoinTestFramework
from test_framework.test_node import (
    HASHVIVE_PID_FILENAME_DEFAULT,
    ErrorMatch,
)
...
class FilelockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second hashvived instance using the same datadir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second hashvived using the same datadir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second hashvived instance shutdown
        pid_file = datadir / HASHVIVE_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second hashvived instance using the same wallet")
                if descriptors:
