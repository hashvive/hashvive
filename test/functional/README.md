...
- The oldest supported Python version is specified in [doc/dependencies.md](/doc/dependencies.md).
  ...
  The CI linter job also checks this, but [possibly not in all cases](https://github.com/hashvive/hashvive/pull/14884#discussion_r239585126).
...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x50=1250 HRLV) in its wallet. Using them is much more
...
- P2P tests have two threads. One thread handles all network communication
  with the hashvived(s) being tested in a callback-based event loop; the other
  implements the test logic.

- `P2PConnection` is the class used to connect to a hashvived. `P2PInterface`
  contains the higher level logic for processing P2P payloads and connecting to
  the Hashvive node application logic. For custom behaviour, subclass the
...
Taken from the [python-hashviverpc repository](https://github.com/jgarzik/python-hashviverpc).
...
Test objects for interacting with a hashvived node over the p2p interface.

#### [script.py](test_framework/script.py)

Utilities for manipulating transaction scripts (originally from python-hashvivelib)
