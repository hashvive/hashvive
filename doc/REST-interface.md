...
_Deprecated (but not removed) since 24.0:_
...
See [BIP64](https://github.com/hashvive/bips/blob/master/bip-0064.mediawiki) for
...
_Query parameters for `verbose` and `mempool_sequence` available in 25.0 and up._

## Risks

Running a web browser on the same node with a REST enabled hashvived can be a risk. Accessing prepared XSS websites could read out tx/block data of your node by placing links like `<script src="http://127.0.0.1:8332/rest/tx/1234567890.json">` which might break the nodes privacy.
