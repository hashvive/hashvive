Hashvive version 0.18.0 is now available from:

<https://hashvive.org/bin/hashvive-core-0.18.0/>
...
`contrib/init/hashvived.service`, it has been changed to use
`/var/lib/hashvived` as the data directory instead of
`~hashvive/.hashvive`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/hashvived` will
exist has enough space (check using `df -h /var/lib/hashvived`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/hashvive/hashvive/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://hashvive.org/en/contact/) the Hashvive
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

## systemd init file

The systemd init file (`contrib/init/hashvived.service`) has been changed
to use `/var/lib/hashvived` as the data directory instead of
`~hashvive/.hashvive`. This change makes Hashvive more consistent with
...
`datadir` option in `/etc/hashvive/hashvive.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/hashvive/hashvive.conf`.

## Documentation

- A new short [document](https://github.com/hashvive/hashvive/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state. A note is added
  to the [REST interface documentation](https://github.com/hashvive/hashvive/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/hashvive/hashvive/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/hashvive/hashvive/blob/master/doc/hashvive-conf.md)
  about the `hashvive.conf` file describes how to use it to configure
  Hashvive.

- A new document introduces Hashvive's BIP174 [Partially-Signed
  hashvive Transactions
  (PSBT)](https://github.com/hashvive/hashvive/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/hashvive/hashvive/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent hashvive-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl. As the payment protocol has exposed
  Hashvive to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/hashvive/hashvive/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/hashvive/hashvive/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Hashvive knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Hashvive, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/hashvive/hashvive/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17. The
  ...
  v0.17](https://github.com/hashvive/hashvive/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart hashvived with the `-deprecatedrpc=generate`
...
- A new `hashvive-wallet` tool is now distributed alongside hashvive
...
This section describes planned changes to Hashvive that may affect
other hashvive software and services.

- Since version 0.16.0, Hashvive’s built-in wallet has defaulted to
  ...
  widely-used software. Starting with Hashvive 0.20 (expected about
  a year after 0.18), Hashvive will default to native segwit
  ...
  to bech32 addresses, and if the Hashvive project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Hashvive 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Hashvive
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers). Previously, Hashvive
...
- A sub-project of Hashvive now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Hashvive. See their [project
  page](https://github.com/hashvive-core/HWI#readme) for details.

## Security

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Hashvive's own implementation, although entropy
  gathered by Hashvive is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Hashvive a
...
- On macOS, Hashvive now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `hashvive_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libhashvive_server.a` (jnewbery)
...
- #13926 [Tools] hashvive-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like hashvived (MarcoFalke)
...
- #14521 qt, docs: Fix `hashvive-qt -version` output formatting (hebasto)
...
- #14204 Move `interfaces/*` to `libhashvive_server` (laanwj)
...
- #15154 Configure: hashvive-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/hashvive-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Hashvive (sipsorcery)
- #15110 build_msvc: Fix the build problem in `libhashvive_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in hashvive-util-test.py (isghe)
...
- #12775 Integration of property based testing into Hashvive (Christewart)
...
- #12246 Bugfix: Only run hashvive-tx tests when hashvive-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_hashvive` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_hashvive` (MarcoFalke)
...
- #14186 hashvive-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_hashvive.conf` message on startup if conf file exists (leishman)
...
- #12255 Update hashvive.service to conform to init.md (dongcarl)
...
- #14497 Add doc/hashvive-conf.md (hebasto)
...
- #15354 Add missing `hashvive-wallet` tool manpages (MarcoFalke)
...
- #15765 correct hashviveconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support hashvive folder on OSX (JimmyMow)
