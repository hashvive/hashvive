...
Hashvive version 0.20.0 is now available from:

<https://hashvive.org/bin/hashvive-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `HASHVIVE_GENBUILD_NO_GIT=1 make`.

# Notable changes

## P2P and network changes

#### Removal of BIP61 reject network messages from Hashvive

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Hashvive version 0.18.0.
...
- Testing or debugging of implementations of the hashvive P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Hashvive. Hashvive logs debug messages
...
- OpenSSL is no longer used by Hashvive. (#17265)

- BIP70 support has been fully removed from Hashvive. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN. See [issue #16599](https://github.com/hashvive/hashvive/issues/16599),
  [PR #16702](https://github.com/hashvive/hashvive/pull/16702), and the
  `hashvived help` for more information. This option is experimental and
...
- All custom settings configured when Hashvive starts are now
...
- The "Start Hashvive on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://hashvive.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed. The issue
  affected anyone who simultaneously used multiple Hashvive wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed hashvive Transaction (PSBT) to
  the system clipboard. This can then be pasted into an external
  program such as [HWI](https://github.com/hashvive-core/HWI) for
  signing. Future versions of Hashvive should support a GUI option
...
- Hashvive's automatically-generated source code documentation is
  now available at https://doxygen.hashvive.org. (#17596)

# Low-level changes

## Utilities

- The `hashvive-cli` utility used with the `-getinfo` parameter now
...
- The `hashvive-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `hashvivestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_hashvive-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBHASHVIVE_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_hashvive` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBHASHVIVE_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for hashvive-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate hashvive schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest hashvive.conf (sdaftuar)
...
- #18022 Fix appveyor `test_hashvive` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (hashvive-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone hashvive-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Hashvive files (hebasto)
...
- #17119 Fix broken hashvive-cli examples (andrewtoth)
...
- #17503 Remove bitness from hashvive-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.hashvive.org (laanwj)
...
- #17648 Rename wallet-tool references to hashvive-wallet (hel-o)
...
- #18341 Replace remaining literal HRLV with `CURRENCY_UNIT` (domob1812)
...
- #16115 On hashvived startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from hashvive-cli -getinfo (malevolent)
