Hashvive version 0.11.0 is now available from:

<https://hashvive.org/bin/hashvive-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer HRLV/kB fee to be
...
For example, add the following to `hashvive.conf`:
...
related to the blockchain in the hashvive system: the raw blocks as received over
the network (blk???.dat), the undo data (rev???.dat), the block index and the
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Hashvive to delete the raw block and undo data once
...
block index and UTXO databases. The minimum was chosen so that Hashvive will
...
For an example script see [hashvive-submittx](https://github.com/laanwj/hashvive-submittx).
...
- #5334 `cf87536` libhashviveconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new hashvive-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add hashvive logo to about screen
...
- #5809 `46bfbe7` Add hashvive-cli man page
...
- #5849 `41259ca` contrib/init/hashvived.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename hashvive-Qt.app to hashvive-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for hashvive-tx)
