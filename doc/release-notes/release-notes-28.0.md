Hashvive version 28.0 is now available from:

<https://hashvive.org/bin/hashvive-core-28.0/>
...
Support for Testnet4 as specified in [BIP94](https://github.com/hashvive/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\hashvive`
to `C:\Users\Username\AppData\Local\hashvive`. Hashvive will check the existence
...
See [JSON-RPC-interface.md](https://github.com/hashvive/hashvive/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
## libhashviveconsensus Removal

The libhashvive-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

## P2P and Network Changes

- Previously if Hashvive was listening for P2P connections, either using
...
- Hashvive will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/hashvive/bips/blob/master/bip-0431.mediawiki). The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Hashvive with the configuration
...
- GCC 11.1 or later, or Clang 16.0 or later,
  are now required to compile Hashvive. (#29091, #30263)

- The minimum required glibc to run Hashvive is now
...
- A new `-testdatadir` option has been added to `test_hashvive` to allow specifying the
  location of unit test data directories. (#26564)

## Blockstorage

- Block files are now XOR'd by default with a key stored in the blocksdir.
  Previous releases of Hashvive or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
