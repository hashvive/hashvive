Hashvive version 0.13.0 is now available from:

<https://hashvive.org/bin/hashvive-core-0.13.0/>
...
- Adding `dbcache=100` in hashvive.conf
- Changing it in the GUI under `Options → Size of database cache`

Note that the database cache setting has the most performance impact
during initial sync of a node, and when catching up after downtime.

## hashvive-cli: arguments privacy

The RPC command line client gained a new argument, `-stdin`
to read extra arguments from standard input, one per line until EOF/Ctrl-D.
For example:

    $ src/hashvive-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Hashvive code base has
...
compatible peers.
[BIP 152](https://github.com/hashvive/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of Hashvive.

[Pull request](https://github.com/hashvive/hashvive/pull/8035/files), [BIP 32](https://github.com/hashvive/bips/blob/master/bip-0032.mediawiki)

## Segregated Witness

The code preparations for Segregated Witness ("segwit"), as described in [BIP
141](https://github.com/hashvive/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/hashvive/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/hashvive/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/hashvive/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release. However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet. Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Hashvive is expected that
...
Witness)] (https://github.com/hashvive/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/hashvive/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the hashvive-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/hashvive/bips/blob/master/bip-0133.mediawiki)
...
- Asm script outputs replacements for OP_NOP2 and OP_NOP3
  ...
  - OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP
    65](https://github.com/hashvive/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP
    112](https://github.com/hashvive/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/hashvive/hashvive/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for hashvive-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use HRLV/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to hashvive-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add hashvive address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: hashvive-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from hashvived (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the hashvive-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for hashvived procs (MarcoFalke)
...
- #8164 `0f24eaf` [hashvive-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of hashvived startup (laanwj)
...
- #7975 `fc23fee` Update hashvive-core GitHub links (MarcoFalke)
