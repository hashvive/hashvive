...
This tutorial will describe how to use two instances of Hashvive, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed hashvive Transactions](https://github.com/hashvive/hashvive/blob/master/doc/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
In this tutorial we have two hosts, both running hashvive v25.0
...
```sh
[offline]$ ./build/src/hashvive-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/src/hashvive-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/src/hashvive-cli -signet -named createwallet \
...
```sh
[online]$ ./build/src/hashvive-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/src/hashvive-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./build/src/hashvive-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/src/hashvive-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/src/hashvive-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/src/hashvive-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/src/hashvive-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
```sh
[offline]$ ./build/src/hashvive-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/src/hashvive-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/src/hashvive-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/src/hashvive-cli -signet -rpcwallet="watch_only_wallet" listtransactions
