# PSBT Howto for Hashvive

Since Hashvive 0.17, an RPC interface exists for Partially Signed hashvive
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/hashvive/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for hashvive transactions that are not fully signed
...
#### Multisig with multiple Hashvive instances
...
Hashvive. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `hashvive-cli` is used.
...
Later, when _V_ HRLV has been received on _Amulti_, and Bob and Carol want to
move the coins in their entirety to address _Asend_, with no change. Alice
