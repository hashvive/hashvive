# User-space, Statically Defined Tracing (USDT) for Hashvive

Hashvive includes statically defined tracepoints to allow for more
...
Is called _after_ a block is connected to the chain. Can, for example, be used
...
**Note**: Hashvive uses temporary clones of the _main_ UTXO cache
...
Is called _after_ the in-memory UTXO cache is flushed.
...
## Adding tracepoints to Hashvive
...
Multiple tools can list the available tracepoints in a `hashvived` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Hashvive, use `info probes` in `gdb`:

```
$ gdb ./build/src/hashvived
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/src/hashvived
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/src/hashvived
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/src/hashvived
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Hashvive.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/src/hashvived | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./build/src/hashvived -v
