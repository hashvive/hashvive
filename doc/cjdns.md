# CJDNS support in Hashvive

It is possible to run Hashvive over CJDNS, an encrypted IPv6 network that
...
done outside of Hashvive, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the hashvive network and individual nodes.
...
before it will work with your Hashvive node. This is described in steps
...
## Run Hashvive with CJDNS

Once you are connected to the CJDNS network, the following Hashvive
...
When enabled, this option tells Hashvive that it is running in an
...
IPv6 local network. This helps Hashvive perform better address management:
...
CJDNS support was added to Hashvive in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `hashvive-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/hashvive/hashvive/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Hashvive

There are several ways to see your CJDNS address in Hashvive:

- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `hashvive-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `hashvive-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `hashvive-cli help getnodeaddresses` for details.
