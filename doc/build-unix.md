...
Some notes on how to build Hashvive in Unix.
...
memory available when compiling Hashvive. On systems with less, gcc can be
...
To build Hashvive without wallet, see [_Disable-wallet mode_](#disable-wallet-mode)
...
...
to: /path/to/hashvive/depends/x86_64-pc-linux-gnu
```

and configure using the following:

```bash
export BDB_PREFIX="/path/to/hashvive/depends/x86_64-pc-linux-gnu"
...
**Note**: You only need Berkeley DB if the legacy wallet is enabled (see [_Disable-wallet mode_](#disable-wallet-mode)).

## Disable-wallet mode

When the intention is to only run a P2P node, without a wallet, Hashvive can
